...
export default function Approveapoint() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const userData = response.data.apointment.find(user => user._id === userId);
  ...
  const handleSendDetails = async () => {
    try {
      ...
      const response = await axios.post("http://localhost:4000/api/v1/updateuserapointment", {
        _id:userId,
        selectedTimeSlot,
        selectedDate,
        selectedTime,
        Response, // Add the Response state as well
      });
